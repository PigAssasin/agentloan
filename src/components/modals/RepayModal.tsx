...
export function RepayModal({ onClose }: { onClose: () => void }) {
  ...
  const MAX_DEBT = "500.00";

  async function handle() {
    const n = parseFloat(amount);
    if (!amount || !isFinite(n) || n <= 0 || n > parseFloat(MAX_DEBT)) return;
    setPending(true);
    ...
    <Modal onClose={onClose}>
      <div style={{ textAlign: "center", padding: "8px 0" }}>
        ...
        <p style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "#999999", marginBottom: 28 }}>
          {amount} USDC repaid successfully
        </p>
  ...
  return (
    <Modal onClose={onClose} title="Repay USDC">
      ...
      <p style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "#999999", marginBottom: 24 }}>
        Outstanding debt: ${MAX_DEBT} USDC
      </p>

      <div style={{ borderTop: "2px solid #000000", paddingTop: 16, marginBottom: 24 }}>
        <OverviewRow label="Remaining debt"  value="$500.00 → $0.00" accent />
        <OverviewRow label="Health factor"   value="4.80 → ∞" />
      </div>
      ...
      >
        {isPending ? "CONFIRMING..." : "REPAY USDC"}
      </button>
    </Modal>
  );
}
