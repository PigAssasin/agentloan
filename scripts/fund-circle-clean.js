...
const ERC20 = parseAbi(['function mint(address to, uint256 amount) external', 'function balanceOf(address) view returns (uint256)']);
const CIRCLE = '0x69efc5abdc9f9f1e90f59261c0fdf601e53291af';
const TOKENS = [
...
async function main() {
  ...
  for (const t of TOKENS) {
    try {
      const h = await wallet.writeContract({ address: t.addr, abi: ERC20, functionName: 'mint', args: [CIRCLE, t.amt] });
      ...
      console.log(t.sym + ' failed: ' + (e.message || '').slice(0, 60));
    }
  }
}
