/**
 * Deploy AgentExecutor v2.1 — adds repayTokenFromWallet for multi-token debt
 * Run: npx hardhat run scripts/deploy-agent-executor-v21.ts --network arcTestnet
 *
 * After deploy: update AGENT_EXECUTOR in config/contracts.ts
 * Users must re-authorize and re-approve tokens to the new address.
 */
...
async function main() {
  ...
  console.log("AgentExecutor v2.1 deployed:", addr);
  ...
  console.log(`2. Users must re-approve tokens to new address`);
  console.log(`3. Users must re-run authorizeAgent(new_addr, true) in pool`);
